...
function buildPrompt(conceptId: string, hasOwnerImage: boolean, hasBackgroundImage: boolean): string {
  ...
  const backgroundInstruction = hasBackgroundImage
    ? `ABSOLUTE RULE — NEVER VIOLATE: use Image ${backgroundImageIndex} ONLY as a reference for the location's architecture, furniture, colors, lighting, and atmosphere. Recreate a similar-looking setting behind the subject(s). Image ${backgroundImageIndex} must have ZERO influence on ${subjectsLabel}'s face or identity — their face, fur pattern, and other identifying features must stay exactly as shown in their source photo, regardless of Image ${backgroundImageIndex}. However, render the background's furniture and room proportions at a scale and perspective that naturally fits around the subjects, as if a photographer actually captured them in that room — adapt the background's scale to match the subjects, not the other way around. The result must look like one coherent, realistically-scaled photograph, not a cutout pasted onto a mismatched background. Do NOT copy any people, pets, animals, text, or objects that already appear in Image ${backgroundImageIndex} into the output — only the pet from Image 1${hasOwnerImage ? ' and the owner from Image 2' : ''} should appear as subjects.`
